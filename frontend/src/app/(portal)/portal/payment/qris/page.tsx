...
import {
  ArrowLeft, QrCode, Clock, Copy, Check, AlertCircle,
  ShieldCheck, Zap, CheckCircle2, Loader2, Download,
} from "lucide-react"
...
function QRISPaymentContent() {
    ...
    const load = async () => {
      try {
        if (orderId) {
          ...
          const selected = (["premium", "pro"].includes(planParam || "") ? planParam : "premium") as PlanKey
          setPlan(selected)
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      if (token) headers["Authorization"] = "Bearer " + token
      try {
        const r = await fetch(`${BASE_API}/v1/payment/qris/status?ref_id=${paymentData.ref_id}`, { headers, credentials: "include" })
  ...
  const handleDownloadQR = useCallback(async () => {
    ...
    try {
      ...
      // Download via backend proxy - menghindari CORS block dari assets.tokopay.id
      ...
      a.href     = url
      a.download = `QRIS-${paymentData.ref_id}.png`
      a.click()
    ...
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4">
        ...
        <div className="text-center">
          ...
          <p className="text-[11px] text-muted-foreground max-w-xs">{error || "Terjadi kesalahan saat membuat pembayaran."}</p>
        </div>
      ...
      <div className="flex flex-row items-center justify-between border-b border-border pb-2.5 gap-2">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col gap-0.5">
            <h1 className="text-lg font-black tracking-tight text-foreground uppercase">QRIS Payment</h1>
              ...
              <>
                ...
                <div className="flex items-center justify-between px-4 pt-4 pb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-extrabold text-xs text-foreground tracking-tight uppercase">Scan QRIS</span>
                  </div>
                ...
                <div className="px-4 pb-3">
                  {paymentData.qr_link ? (
                    ...
                    <div className="rounded-lg border border-dashed border-border bg-muted/10 p-8 flex flex-col items-center gap-3">
                      <QrCode className="w-16 h-16 text-muted-foreground/20" />
                      <p className="text-[11px] text-muted-foreground text-center">QR tidak tersedia untuk order ini</p>
                    </div>
                  )}
                </div>
                ...
                {/* Action buttons */}
                  ...
                  <div className="px-4 pb-3 flex gap-2">
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg border border-border bg-muted/30 hover:bg-muted/60 text-[11px] font-semibold text-foreground transition-colors disabled:opacity-50 cursor-pointer"
                    >
                      {downloading
            ...
            <div className="rounded-xl border border-border bg-card p-4 flex flex-col gap-3 shadow-xs">
              ...
              <ol className="flex flex-col gap-2.5">
                {[
                  ...
                  "Scan QR Code yang tampil di atas",
                  `Konfirmasi nominal: ${formatIDR(displayPrice)}`,
                  "Lisensi aktif otomatis setelah pembayaran terverifikasi",
                ].map((step, i) => (

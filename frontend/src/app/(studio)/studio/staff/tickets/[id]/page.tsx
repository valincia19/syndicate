...
export default function StaffTicketDetailPage() {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-20">
        ...
        <span className="ml-2 text-[11px] font-mono text-muted-foreground">Loading conversation...</span>
      </div>
    )
  }
  ...
  return (
    <>
      <style>{`
        .staff-chat-scrollbar::-webkit-scrollbar { width: 6px; height: 6px; }
        .staff-chat-scrollbar::-webkit-scrollbar-track { background: transparent; }
        .staff-chat-scrollbar::-webkit-scrollbar-thumb { background-color: var(--border); border-radius: 9999px; }
        .staff-chat-scrollbar::-webkit-scrollbar-thumb:hover { background-color: var(--muted-foreground); }
        .staff-chat-scrollbar { scrollbar-width: thin; scrollbar-color: var(--border) transparent; }
      `}</style>
      <div className="flex flex-col h-[calc(100vh-220px)] max-w-4xl mx-auto">
          ...
          <div className="flex flex-row items-start justify-between border-b border-border pb-2.5 gap-2">
            <div className="flex flex-col gap-0.5 min-w-0">
              ...
              <p className="text-[11px] text-muted-foreground">
                ...
                {ticket.user_email && !ticket.user_username && (
                  <span> by <span className="font-mono text-foreground font-bold">{ticket.user_email}</span></span>
                )}
              </p>
            </div>
        ...
        <div className="flex-1 overflow-y-auto py-3 space-y-3 min-h-0 staff-chat-scrollbar">
                  ...
                  <div className={`rounded-lg border px-3.5 py-2.5 ${
                    ...
                    <div className="flex items-center gap-2 mb-1.5">
                      <span className="text-[9px] font-mono font-bold text-foreground">
                        {isOwnMessage ? 'You' : (msg.sender_username || 'Unknown')}
                      </span>
        ...
        {ticket.status !== 'closed' ? (
          <div className="shrink-0 pt-3">
            <div className="rounded-lg border border-border bg-card p-4 shadow-xs space-y-3">
              <h3 className="font-extrabold text-xs text-foreground tracking-tight uppercase">Staff Reply</h3>
              ...
              <Textarea
                rows={3}
                placeholder="Type your staff reply... (user will see this)"
                value={replyContent}
          ...
          <div className="shrink-0 pt-3">
            <div className="flex items-center justify-center gap-2 px-4 py-3 rounded-lg bg-muted/30 border border-border text-[10px] text-muted-foreground font-mono">
              ...
              <span>This ticket is closed and no longer accepts replies. Create a new ticket if further assistance is needed.</span>
            </div>
          </div>
        )}
      </div>

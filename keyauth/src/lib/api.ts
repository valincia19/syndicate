...
export const api = {
   ...
   * GET /v1/bypass/session/:session_id
   */
  async checkSession(sessionId: string): Promise<boolean> {
    try {
      const res = await fetch(`${BASE_URL}/v1/bypass/session/${sessionId}`);
      return res.ok;
    } catch {
      return false;
    }
  ...
  /**
   * Check if a license key is valid (exists in database and not expired).
   * GET /v1/bypass/check-key/:key
   */
  async checkKey(key: string): Promise<boolean> {
    try {
      const res = await fetch(`${BASE_URL}/v1/bypass/check-key/${key}`);
      return res.ok;
    } catch {
      return false;
    }

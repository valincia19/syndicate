...
import {
  ArrowLeft, Store, Clock, Copy, Check, AlertCircle,
...
function getRetailLabel(code: string | null): string {
  if (!code) return "Over-the-Counter Retail"
  const clean = code.toUpperCase()
  if (clean === "ALFAMART") return "Alfamart Gerai Retail"
  if (clean === "INDOMARET") return "Indomaret Gerai Retail"
  return clean
}

function getRetailInstructions(code: string | null, codeBayar: string, amountStr: string): string[] {
  const channel = (code || "").toUpperCase()
  switch (channel) {
    case "ALFAMART":
      return [
        "Kunjungi gerai Alfamart terdekat sebelum waktu pembayaran habis.",
        "Katakan pada kasir bahwa Anda ingin melakukan pembayaran 'Tokopay' atau 'Payment Link'.",
        `Tunjukkan Kode Pembayaran ini kepada kasir: ${codeBayar}`,
        `Konfirmasi bahwa transaksi adalah untuk VALINC SYNDICATE dengan nominal sebesar ${amountStr}.`,
        "Bayar sesuai jumlah tagihan tersebut dan simpan struk pembayaran dari kasir."
      ]
    case "INDOMARET":
      return [
        "Kunjungi gerai Indomaret terdekat sebelum waktu pembayaran habis.",
        "Katakan pada kasir bahwa Anda ingin melakukan pembayaran tagihan 'Tokopay'.",
        `Tunjukkan Kode Pembayaran ini kepada kasir: ${codeBayar}`,
        `Konfirmasi detail tagihan atas nama VALINC SYNDICATE dengan nominal sebesar ${amountStr}.`,
        "Selesaikan pembayaran Anda di kasir dan simpan struk transaksinya."
      ]
    default:
      return [
        "Kunjungi gerai retail Alfamart atau Indomaret terdekat.",
        "Katakan pada kasir Anda ingin melakukan pembayaran tagihan 'Tokopay'.",
        `Tunjukkan Kode Pembayaran berikut kepada kasir: ${codeBayar}`,
        `Bayar sesuai jumlah nominal tepat sebesar ${amountStr}.`,
        "Simpan struk pembayaran sebagai bukti transaksi pembayaran."
      ]
  }
}

function RetailPaymentContent() {
  const searchParams = useSearchParams()
  const router = useRouter()

  const orderId   = searchParams.get("orderId")
  const planParam = searchParams.get("plan")
  const retailParam = searchParams.get("retail") // e.g. ALFAMART
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        if (orderId) {
          const res  = await fetch(`${BASE_API}/v1/payment/retail/order?ref_id=${orderId}`, { headers, credentials: "include" })
          ...
          const selectedRetail = retailParam || "ALFAMART"
          setPlan(selectedPlan)

          const res  = await fetch(`${BASE_API}/v1/payment/retail/create-order`, {
            method: "POST", headers, credentials: "include",
            body: JSON.stringify({ plan: selectedPlan, retail: selectedRetail }),
          })
          if (res.status === 401) { if (isMounted) router.push("/login"); return }
          const json = await res.json()
          if (!isMounted) return
  ...
  }, [orderId, planParam, retailParam, router])
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      if (token) headers["Authorization"] = "Bearer " + token
      try {
        const r = await fetch(`${BASE_API}/v1/payment/retail/status?ref_id=${paymentData.ref_id}`, { headers, credentials: "include" })
    ...
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4 max-w-sm mx-auto text-center">
        ...
        <div>
          ...
          <p className="text-[11px] text-muted-foreground leading-relaxed">{error || "Terjadi kesalahan saat memuat detail retail."}</p>
        </div>
      ...
      <div className="flex flex-row items-center justify-between border-b border-border pb-2.5 gap-2">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col gap-0.5">
            <h1 className="text-lg font-black tracking-tight text-foreground uppercase">Retail Payment</h1>
      ...
      <div className="grid gap-3 lg:grid-cols-3 items-start">
        ...
        <div className="lg:col-span-2 flex flex-col gap-3">
          ...
          <div className="rounded-xl border border-border bg-card shadow-xs overflow-hidden">
            ...
            {/* ── Pending: Detail Pembayaran ── */}
            {paymentStatus === "pending" && (
              <>
                <div className="flex items-center justify-between px-4 pt-4 pb-2 border-b border-border">
                  <div className="flex items-center gap-2">
                    <Store className="w-4 h-4 text-primary" />
                    <span className="font-extrabold text-xs text-foreground tracking-tight uppercase">Kode Pembayaran Retail</span>
                  </div>
                  <span className="text-[9px] font-mono text-primary bg-primary/10 border border-primary/20 px-2 py-0.5 rounded font-bold uppercase tracking-wider">
                    {getRetailLabel(paymentData.bank_code)}
                  </span>
                </div>
                ...
                <div className="p-4 flex flex-col gap-3">
                  ...
                  <div className="flex flex-col gap-1.5 p-3 rounded-lg border border-border bg-muted/20">
                    <span className="text-[9px] font-mono text-muted-foreground/60 uppercase tracking-widest">Kode Pembayaran</span>
                    {paymentData.va_number ? (
                      ...
                      <span className="text-xs text-muted-foreground">Generating Payment Code...</span>
                    )}
                  </div>

                  {/* Nominal Row */}
                  <div className="flex flex-col gap-1.5 p-3 rounded-lg border border-border bg-muted/20">
                    <span className="text-[9px] font-mono text-muted-foreground/60 uppercase tracking-widest">Nominal Pembayaran</span>
                    <div className="flex items-center justify-between gap-2">
          ...
          {paymentStatus === "pending" && paymentData.va_number && (
            <div className="rounded-xl border border-border bg-card p-4 flex flex-col gap-3 shadow-xs">
              ...
              <ol className="flex flex-col gap-2.5">
                {getRetailInstructions(paymentData.bank_code, paymentData.va_number, formatIDR(displayPrice)).map((step, i) => (

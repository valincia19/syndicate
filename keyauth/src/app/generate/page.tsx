...
function GenerateKeyContent() {
  ...
  const checkAdBlocker = useCallback(async () => {
    try {
      await fetch("/adsbygoogle.js", { method: "HEAD", cache: "no-store" });
      setAdblockDetected(false);
    } catch {
      setAdblockDetected(true);
      setError("AdBlocker Terdeteksi! Harap matikan AdBlock Anda demi kelancaran Get Key.");
      toast.error("AdBlocker Terdeteksi! Harap matikan AdBlock Anda demi kelancaran Get Key.", { duration: 8000 });
    }
  }, [setError]);

  useEffect(() => {
    setTimeout(() => setMounted(true), 0);
    setTimeout(() => checkAdBlocker(), 0);
  }, [checkAdBlocker]);

  const cleanSessionId = useCallback((id: string | null) => {
    if (!id) return "";
  ...
  useEffect(() => {
    ...
    if (querySessionId && querySessionId !== "null" && querySessionId !== "undefined" && querySessionId.trim() !== "") {
      resolvedSessionId = cleanSessionId(querySessionId);
      
      // Strict browser verification: check if this matches the originally stored session ID in this browser
      let storedId = "";
      ...
      if (!storedId) {
        setError(t("errorBrowserSwitchNew"));
        setSettingsLoaded(true);
        return;
      }

      if (storedId !== resolvedSessionId) {
        setError(t("errorBrowserSwitchSession"));
        setSettingsLoaded(true);
        return;
      }
      ...
      const storedId = localStorage.getItem("keyauth_current_session_id");
      const storedUuid = localStorage.getItem("keyauth_current_session_uuid");
      if (storedId && storedId !== "null" && storedId !== "undefined" && storedId.trim() !== "") {
        resolvedSessionId = storedId;
        setSessionId(storedId);
      }
      if (storedUuid && storedUuid !== "null" && storedUuid !== "undefined" && storedUuid.trim() !== "") {
    ...
    const loadSettings = async () => {
      try {
        // Validate if the stored session is still active in Redis (skip during callback redirects)
        if (resolvedSessionId && !isCallback) {
          const isValid = await api.checkSession(resolvedSessionId);
          if (!isValid) {
            resolvedSessionId = "";
            setSessionId(null);
            setSessionUuid(null);
            if (typeof window !== "undefined") {
              localStorage.removeItem("keyauth_current_session_id");
              localStorage.removeItem("keyauth_current_session_uuid");
            }
          }
        }
        ...
        if (!resolvedSessionId && !sessionCreatedRef.current && !sessionId) {
          ...
          try {
            ...
            if (sess.data) {
              ...
              if (typeof window !== "undefined") {
                localStorage.setItem("keyauth_current_session_id", sess.data.session_id);
                localStorage.setItem("keyauth_current_session_uuid", sess.data.uuid);
              }
            }
          } catch (err) {
            const msg = err instanceof ApiError ? err.message : "Failed to create verification session. Please reload the page.";
            setError(msg);
        ...
        toast.error("Failed to load settings. Please check your connection and reload.");
        void err;
      } finally {
        setSettingsLoaded(true);
      }
  ...
  }, [isCallback, searchParams, cleanSessionId, sessionId, t]);

  // ── Claim Key function ───────────────────────────────────
  const handleClaimKey = useCallback(async (forcedSessionId?: string) => {
    if (claiming) return;
    setClaiming(true);
    setError(null);

    const activeSessionId = forcedSessionId || cleanSessionId(searchParams.get("session_id") || sessionId);
    ...
    try {
      ...
      router.push(`/success/${encodeURIComponent(newKey.id)}`);
    } catch (err) {
      ...
      try {
        ...
        if (stored) {
          ...
          if (parsed.length > 0) {
            ...
            router.push(`/success/${encodeURIComponent(parsed[0].id || parsed[0].license_key)}`);
            return;
          }
        }
      } catch {}
  ...
  }, [t, claiming, router, searchParams, sessionId, cleanSessionId, setError]);

  // Automatically request key if landing back via shortlink callback
  useEffect(() => {
    if (mounted && settingsLoaded && isCallback && !claimedKey && !error && !claiming && !callbackProcessedRef.current) {
      callbackProcessedRef.current = true;
      
      let activeSessionId = cleanSessionId(searchParams.get("session_id") || sessionId);
      ...
      const processCallback = async () => {
        if (linkvertiseHash && activeSessionId) {
          try {
            ...
            // Server confirmed shortlink — only NOW mark step as completed
            setShortlinkStatus("completed");
          } catch (err) {
            const msg = err instanceof ApiError
              ? (err.message || "Linkvertise anti-bypass verification failed. Please complete the shortlink legally.")
              : "Linkvertise anti-bypass verification failed.";
          ...
          try {
            ...
            const msg = err instanceof ApiError
              ? (err.message || "Work.ink anti-bypass verification failed. Please complete the shortlink legally.")
              : "Work.ink anti-bypass verification failed.";
            toast.error(msg, { duration: 6000 });
            setError(msg);
            return;
          }
        ...
        handleClaimKey(activeSessionId);
      };

      processCallback();
    }
  }, [mounted, settingsLoaded, isCallback, claimedKey, error, claiming, handleClaimKey, searchParams, sessionId, cleanSessionId]);

  // ── Captcha Verify Callback ─────────────────────────────
  const handleCaptchaVerify = useCallback(async (token: string) => {
    _setCaptchaToken(token);
    const activeSessionId = cleanSessionId(searchParams.get("session_id") || sessionId);
    if (!activeSessionId) {
      setCaptchaStatus("completed");
      return;
    }
    try {
      ...
      const msg = err instanceof ApiError
        ? (err.message || "Captcha verification failed on server. Please try again.")
        : "Captcha verification failed on server. Please try again.";
      toast.error(msg, { duration: 5000 });
      setError(msg);
    }
  }, [searchParams, sessionId, cleanSessionId, setError]);

  // ── Captcha Error Callback ──────────────────────────────
  const handleCaptchaError = useCallback((errCode: unknown) => {
    const msg = `Captcha failed to load (${errCode || 'Unknown Error'}). Please ensure your connection is stable and try reloading.`;
    toast.error(msg, { duration: 8000 });
    setError(msg);
  }, [setError]);

  // ── Shortlink Click Callback ─────────────────────────────
  // SECURITY: Do NOT call setShortlinkStatus("completed") here.
  // The VERIFIED badge must only appear after the server has confirmed the
  // shortlink via API (on callback return). Setting it here would let any
  // user with React DevTools fake a completed state without going through
  // the actual shortlink. The real claim-gate is the `isCallback` URL param
  // and the Redis session validated by the server.
  const handleOpenProvider = useCallback((name: string, url: string) => {
    ...
    // Redirect current tab to target URL (short delay for UX feedback)
    setTimeout(() => {
      window.location.href = targetUrl;
    }, 600);
  }, [searchParams, sessionId, sessionUuid, cleanSessionId]);
  ...
  const handleReset = useCallback(() => {
    ...
    // Clear session storage so we get a fresh session
    if (typeof window !== "undefined") {
      localStorage.removeItem("keyauth_current_session_id");
      localStorage.removeItem("keyauth_current_session_uuid");
    }
    ...
    sessionCreatedRef.current = false;
    checkAdBlocker();
  }, [checkAdBlocker, setError, setAdblockDetected]);

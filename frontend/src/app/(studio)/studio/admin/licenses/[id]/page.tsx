...
import {
  ...
  Trash2, Monitor, Loader2, Fingerprint, RotateCcw, Gift
} from "lucide-react"
...
function formatDate(d: string | null): React.ReactNode {
  if (!d) return "-"
  return <ClientDate date={d} format="datetime" />
}

function formatShortDate(d: string | null): React.ReactNode {
  if (!d) return "-"
...
export default function AdminLicenseDetailPage() {
  ...
  const handleResetDevice = async (hwidId: string) => {
    ...
    try {
      ...
      const errorMsg = (err as { message?: string }).message || "Failed to reset device HWID"
      alert(errorMsg)
    } finally {
  ...
  const handleUnlinkDevice = async (hwidId: string) => {
    ...
    try {
      ...
      const errorMsg = (err as { message?: string }).message || "Failed to unlink device"
      alert(errorMsg)
    } finally {
      setUnlinkingId(null)
    }
    ...
    return (
      <div className="max-w-2xl mx-auto">
        <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-8 text-center">
          ...
          <Link href="/studio/admin/licenses" className="inline-flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground text-[11px] font-mono rounded-lg hover:opacity-90 transition-all font-bold cursor-pointer">
    ...
    <div className="max-w-3xl mx-auto space-y-3">
      ...
      <div className="flex flex-row items-center justify-between border-b border-border pb-2.5 gap-2">
        ...
        <div className="flex items-center gap-2">
          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-sm border text-[9px] font-mono ${license.source === "redeem" ? "bg-emerald-500/5 text-emerald-600 border-emerald-500/20" : "bg-muted/30 text-muted-foreground border-border/50"}`}>
            {license.source === "redeem" ? <Gift className="w-3 h-3" /> : null}
            {license.source === "redeem" ? "Redeem" : "Admin"}
          </span>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="rounded-lg border border-border bg-card p-3 flex flex-col gap-2 shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-[9px] font-bold uppercase tracking-wider text-muted-foreground">Owner</span>
            <span className="text-[10px] font-mono text-muted-foreground/65">User Profile</span>
          </div>
          <span className="text-sm font-mono font-black text-foreground tracking-tight truncate" title={license.user_email || "No email"}>
            {license.user_name || license.user_email || "-"}
          </span>
        </div>
      </div>
      ...
      <div className="rounded-lg border border-border bg-card shadow-xs overflow-hidden">
        <div className="px-4 py-2.5 border-b border-border flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-[11px] font-bold text-foreground uppercase tracking-wider">License Information (Admin View)</h3>
          </div>
        </div>
        <div className="divide-y divide-border/60">
          ...
          <div className="flex items-center justify-between px-4 py-2.5">
            <span className="text-[10px] font-mono text-muted-foreground">Owner ID</span>
            <span className="text-[10px] font-mono text-muted-foreground/60">{license.user_id || "-"}</span>
          </div>
          <div className="flex items-center justify-between px-4 py-2.5">
            <span className="text-[10px] font-mono text-muted-foreground">Owner Name</span>
            <span className="text-[10px] font-mono font-bold text-foreground">{license.user_name || "-"}</span>
          </div>
          <div className="flex items-center justify-between px-4 py-2.5">
            <span className="text-[10px] font-mono text-muted-foreground">Owner Email</span>
            <span className="text-[10px] font-mono font-bold text-foreground">{license.user_email || "-"}</span>
          </div>
          <div className="flex items-center justify-between px-4 py-2.5">
          ...
          <div className="flex items-center justify-between px-4 py-2.5">
            <span className="text-[10px] font-mono text-muted-foreground">Creation Source</span>
            <span className="text-[10px] font-mono font-bold text-foreground uppercase">{license.source}</span>
          </div>
          <div className="flex items-center justify-between px-4 py-2.5">
            <span className="text-[10px] font-mono text-muted-foreground">Created At</span>
            <span className="text-[10px] font-mono text-muted-foreground/60">{formatDate(license.created_at)}</span>
          </div>
          <div className="flex items-center justify-between px-4 py-2.5">
                      ...
                      <div className="flex items-center gap-2.5">
                        <div className="w-7 h-7 rounded-full bg-muted border border-border overflow-hidden shrink-0">
                          {d.roblox_avatar ? (
                            ...
                            <img src={d.roblox_avatar} alt="" className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; if ((e.target as HTMLImageElement).parentElement) { (e.target as HTMLImageElement).parentElement!.innerText = d.roblox_username.charAt(0).toUpperCase() } }} />
                          ) : (
